...
def label_to_onehot(label, n_class=10):
    one_hot = torch.eye(n_class)
    return one_hot[label.long()]    
    ...
    for epoch in range(n_epochs):
        ...
        for i, ((train_data, train_lbls),
            ...
            if type(shadow_model) is not Pipeline:
                ...
                train_posteriors = F.softmax(shadow_model(train_data),
                                                 dim=1)
                out_posteriors = F.softmax(shadow_model(out_data),
                                               dim=1)
...
class softCrossEntropy(torch.nn.Module):
    ...
    def forward(self, input, target, true_labels):
        KD_loss = self.alpha
        KD_loss *= nn.KLDivLoss(size_average=False)(
            F.log_softmax(input, dim=1), F.softmax(target, dim=1))
        KD_loss += (1 - self.alpha) * F.cross_entropy(input, true_labels)
...
def inf_adv_train(target_model=None, inf_model=None, train_set=None,
    ...
    for epoch in range(n_epochs):
        ...
        for k_count, ((in_data, _), (out_data, _)) in enumerate(zip(inf_in_set,
            ...
            train_posteriors = F.softmax(target_model(in_data), dim=1)
            out_posteriors = F.softmax(target_model(out_data), dim=1)
