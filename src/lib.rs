...
fn generate_mnist_proof() {
    ...
    let end = Instant::now();
    println!("prove time {:?}", end.duration_since(begin));
    let x_inputs: Vec<Fq> = full_circuit::convert_2d_vector_into_fq(x_current_batch.clone());
    let true_label_inputs: Vec<Fq> = convert_1d_vector_into_fq(true_labels_batch.clone());
...
fn zkfl(_py: Python, _m: &PyModule) -> PyResult<()> {
    ...
    _m.add_function(wrap_pyfunction!(generate_mnist_proof, _m)?)?;
    Ok(())
}
